...
\begin{definition}[Checker Language]
    ...
    \uses{def:CheckerIntermediateLanguage,def:Checker}
     Let $C$ be a checker. $w$ is a member of the Checker Language $\lang$ if and only if $w \in \langint$ and $\$ \in C(w)$. \end{definition}
...
\begin{definition}[Checker Sound]
    ...
    \uses{def:Checker}
    A checker is \emph{sound} if $w \in \langint$ implies that there exists some $w' \in \lang$ such that $w$ is a prefix of $w'$. Informally, you never run into a case where a language model has no choices, assuming that it follows constrained decoding.
\end{definition}
...
\begin{theorem}[Checker Complete implies Sound]
    \label{def:CheckerCompleteImplesSound}
    \uses{def:CheckerComplete,def:CheckerSound}
